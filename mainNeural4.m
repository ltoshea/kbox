...
%close all
for i=1:PNUM
    dataAll(i).jred=reconstructPose(dataAll(i).testdata,Xm1,EV1);
...
for i=1:PNUM
    ...
    plot(dataAll(i).testimax, dataAll(i).jredSmooth(1,dataAll(i).testimax),'.g');
end
...
for i=1:PNUM
   ...
   X = [X;dataAll(i).testfeatures]; 
   Y = [Y;dataAll(i).testlabels];
