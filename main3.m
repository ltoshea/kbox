...
for i=1:PNUM
    ...
    %data = loadKinectData2(path); Will return a matrix for each punch.
    %data = loadKinectData3(path); %2 will return a cell of all files.
...
for i=1:PNUM*length(data)

    %Create Eignen Spaces for Both
    [Xm1,EV1,Ev1]=createES(C{1,1}{i,1},3);

    %Reconstruct the data - Output Principal components.
    jred=reconstructPose(C{1,1}{i,1},Xm1,EV1);

    %Smooths PC1,PC2,PC3 and outputs. Removes rest of data.
    C{1,1}{i,1} = kinsmooth(jred);
    % Min/Max of smoothed curve
    [zmax,zmin,imax,imin] = getminmax(C{1,1}{i,1});
...
%     SPP = 40;
%     framenum = linspace(maxima(k,1),maxima(k+1,k+1),SPP);
