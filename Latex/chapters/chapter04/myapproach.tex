...
\begin{figure}[h]
    ...
    \mycaption[Position of the left wrist joint over time]{This shows the left wrist joints movement in the z direction over time. The x-axis represents time (kinect frames) and the y-axis the distance of the fist from the Kinect}
\end{figure}
...
\qquad
\end{flushleft}
\mycaption[Comparison of original pose and reconstructed pose]{Each graph represents a single pose with the original data in green and the reconstructed data in red obtained from back propagation. We can see from the reconstruction how much data has been lost as a result of dimensionality reduction.
...
\begin{center}
    ...
    % \hline
    \end{tabular}
\mycaption [SVM testing]{Punch classification accuracy of different numbers of samples for SVM on a six class problem using a reduced dataset (Jab, Cross, Left-Hook, Right-Hook, Left-Uppercut, Right-Uppercut)}
\end{center}
\end{table}
...
\begin{minipage}{6.0cm}
    ...
    \label{fig:2}
\end{minipage}
\mycaption[Pose representation using distance and velocity]{Distance data is shown in blue and velocity data shown in green.
