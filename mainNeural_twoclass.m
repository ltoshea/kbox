...
for i=1:PNUM
    ...
    [valmax,imax,valmin, imin] = getminmax(dataAll(i).jredSmooth(1,:),1,NORM); %ERROR, BUG, CHANGE THIIS
...
labels1 = repmat(lbl1,length(dataAll(1).labels),1);
labels2 = repmat(lbl2,length(dataAll(2).labels),1);
%labels = [labels; zeros(length(dataAll(2).labels),1)];
